...
function logUpstream(
  ...
  const isError =
    direction === "✗" || (extra.status !== undefined && extra.status >= 400);
  ...
  if (extra.body !== undefined) {
    console.log(
      "        body:",
      JSON.stringify(redactBody(extra.body), null, 2),
    );
  }
}
...
async function doRequest<TData, TBody>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[BFF] Upstream error on ${opts.service}${opts.path}: ${response.status}`,
      );
      let details: unknown;
      try {
    ...
    if (err instanceof DOMException && err.name === "AbortError") {
      ...
      throw new AppError(
        "timeout",
        `Timeout calling ${opts.service}${opts.path}`,
        null,
        err,
      );
    }
