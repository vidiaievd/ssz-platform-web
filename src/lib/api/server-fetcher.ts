...
export type ServerFetchOptions<TBody = unknown> = {
  ...
  /**
   * Upstream statuses that are a normal outcome for this call, not a fault.
   * They still reject with an AppError so the caller branches on them, but
   * they are not logged as errors.
   * @example [403, 404]   // probing an endpoint the user may not be allowed
   */
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (!opts.expectedErrorStatuses?.includes(response.status)) {
        console.warn(`[BFF] Upstream error on ${opts.service}${opts.path}: ${response.status}`);
      }
      let details: unknown;
      try {
      ...
      throw new AppError(
        code,
        `Upstream ${response.status} on ${opts.service}${opts.path}`,
        details,
      );
    }
    ...
    throw new AppError(
      ...
      `Upstream call failed: ${opts.service}${opts.path}`,
      null,
      err,
    );
  } finally {
