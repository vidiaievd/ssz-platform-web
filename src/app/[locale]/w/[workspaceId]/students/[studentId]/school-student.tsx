...
export async function SchoolStudentCard({ params, searchParams }: Props) {
  ...
  const assignHref = wsHref(workspaceId, `students/${studentId}/assign-group`);
    ...
    <main className="p-4 sm:p-6 lg:p-8 max-w-page mx-auto space-y-5">
      ...
      {tab === "overview" && (
        <OverviewTab
          ...
          schoolId={school.id}
          canEdit={isOwner}
          groupsHref={wsHref(workspaceId, `students/${studentId}?tab=groups`)}
        />
      )}
      {tab === "groups" && (
        <GroupsTab
      ...
      {tab === "mastery" && (
        <MasteryTab
          ...
          groups={groups}
          assignHref={wsHref(workspaceId, 'groups')}
        />
      )}
      {tab === "history" && (
        <HistoryTab student={student} workspaceId={workspaceId} />
      )}
      {tab === "notes" && (
        <StubTab
          icon={StickyNote}
