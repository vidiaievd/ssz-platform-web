...
describe('POST /api/content/lessons/[id]/variants/[variantId]/paragraphs', () => {
  it('replaces the paragraph translations and returns 204', async () => {
    vi.mocked(serverFetch).mockResolvedValueOnce(undefined);
    const translations = [{ paragraphIndex: 0, translation: 'Hi, how are you?' }];

    const res = await POST(makePostRequest({ translations }), PARAMS);
