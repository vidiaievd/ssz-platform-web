...
type MembersPayload = Array<{ userId: string; role?: SchoolRole; joinedAt?: string }>;
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  let school = await getSchool(id);

  if (!school && !UUID_RE.test(id)) {
    const schools = await getMySchools();
    school = schools.find((s) => s.slug === id) ?? null;
  }
  ...
  if (!school.members) {
    try {
      ...
      schoolForRole = { ...school, members: Array.isArray(membersRaw) ? membersRaw : [] };
    } catch {
  ...
  const [kpis, atRisk, courseHealth, activity, membersResult, containersResult, reviewResult] =
    await Promise.all([
      ...
      fetchCount<MembersPayload>(() =>
        serverFetch({ service: 'organization', path: `/schools/${schoolId}/members` }),
      ),
      ...
      fetchCount<ReviewPayload>(() =>
        serverFetch({ service: 'progress', path: '/review/submissions/pending', query: { schoolId, limit: 1 } }),
      ),
    ]);
  ...
  const hasActivity =
    'items' in activity && Array.isArray(activity.items) && activity.items.length > 0;
...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  try {
    const data = await serverFetch({
      service: 'analytics',
      path: `/schools/${id}/dashboard/nudge`,
      method: 'POST',
      body,
    });
    ...
    return NextResponse.json({ error: 'Failed to nudge students' }, { status: 502 });
  }
}
...
async function fetchCount<T>(fn: () => Promise<T>): Promise<number | null> {
  try {
    ...
    if (data && typeof data === 'object') {
      if ('total' in data && typeof (data as Record<string, unknown>).total === 'number') {
