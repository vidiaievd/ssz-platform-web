...
/** POST /api/schools/[id]/invitations/[invitationId]/resend */
export async function POST(_req: Request, { params }: Params) {
  const { id, invitationId } = await params;
  try {
    const provider = getInvitationsProvider();
    const result = await provider.resend(id, invitationId);
    ...
    return NextResponse.json({ error: 'Failed to resend invitation' }, { status: 502 });
  }
}
