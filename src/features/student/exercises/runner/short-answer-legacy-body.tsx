...
/**
 * The single-question `short_answer`, graded in the browser against a list of accepted
 * strings — the form 144 seeded exercises are still written in (plan 51 §8 Q1).
 *
 * Kept whole and untouched beside the new runner rather than migrated, for the same
 * reason `short-answer-legacy.ts` is kept on the server: those documents hold no
 * semantic key to grade against, and the reader dispatches on the shape of the document
 * it loaded. It goes when the catalogue is rewritten, not before.
 */
...
export interface ShortAnswerLegacyExpectedAnswers {
...
export interface ShortAnswerLegacyBodyProps {
  content: ShortAnswerLegacyContent;
...
export function ShortAnswerLegacyBody({
  content,
  value,
  onValueChange,
  onAnswerChange,
