...
export interface MultipleChoiceAutosave {
  status: SaveStatus;
 ...
 */
export function sameDocument(a: MultipleChoiceDocument, b: MultipleChoiceDocument): boolean {
  if (a === b) return true;
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]) as Set<keyof MultipleChoiceDocument>;
 ...
 */
export function useMultipleChoiceAutosave({
  exerciseId,
  containerId,
  exercise,
  onSaved,
}: Options): MultipleChoiceAutosave {
  ...
  const flush = useCallback(
    async (force?: { expectedUpdatedAt: string }) => {
      ...
      const outcome: SaveMultipleChoiceOutcome = result.value;
