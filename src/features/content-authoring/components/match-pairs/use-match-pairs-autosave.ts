...
export interface MatchPairsAutosave {
  status: SaveStatus;
 ...
 */
function sameDocument(a: MatchPairs, b: MatchPairs): boolean {
  if (a === b) return true;
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]) as Set<keyof MatchPairs>;
 ...
 */
export function useMatchPairsAutosave({
  exerciseId,
  containerId,
  exercise,
  instructions,
  ...
  const flush = useCallback(
    async (force?: { expectedUpdatedAt: string }) => {
      ...
      const outcome: SaveMatchPairsOutcome = result.value;
