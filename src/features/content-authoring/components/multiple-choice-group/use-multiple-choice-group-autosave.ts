...
export interface MultipleChoiceGroupAutosave {
  status: SaveStatus;
...
export function sameDocument(
  a: MultipleChoiceGroupDocument,
  b: MultipleChoiceGroupDocument,
): boolean {
  if (a === b) return true;
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]) as Set<
    keyof MultipleChoiceGroupDocument
  >;
 ...
 *
 * A copy of `use-multiple-choice-autosave.ts` in everything that matters, because what
 ...
 */
export function useMultipleChoiceGroupAutosave({
  exerciseId,
  containerId,
  exercise,
  onSaved,
}: Options): MultipleChoiceGroupAutosave {
  ...
  const flush = useCallback(
    async (force?: { expectedUpdatedAt: string }) => {
      ...
      const outcome: SaveMultipleChoiceGroupOutcome = result.value;
