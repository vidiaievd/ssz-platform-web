...
export interface ShortAnswerAutosave {
  status: SaveStatus;
 ...
 */
export function sameDocument(a: ShortAnswerDocument, b: ShortAnswerDocument): boolean {
  if (a === b) return true;
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]) as Set<keyof ShortAnswerDocument>;
 ...
 *
 * A copy of `use-writing-task-autosave.ts` in everything that matters, because what
 ...
 */
export function useShortAnswerAutosave({
  exerciseId,
  containerId,
  exercise,
  onSaved,
}: Options): ShortAnswerAutosave {
  ...
  const flush = useCallback(
    async (force?: { expectedUpdatedAt: string }) => {
      ...
      const outcome: SaveShortAnswerOutcome = result.value;
