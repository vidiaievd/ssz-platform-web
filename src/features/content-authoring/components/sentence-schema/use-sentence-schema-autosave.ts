 ...
 *
 * A blip on the wire and a document the server will not accept look identical to a caller
 * — both arrive as a failed action — but they need opposite treatment. Backoff is right
 * for the blip and wrong for the refusal: the same request re-sent every thirty seconds
 * cannot start succeeding, and while it loops the author is told "your edits are still
 * here" over and over, which reads as a network problem that will pass. It will not.
 */
...
export interface SentenceSchemaAutosave {
  status: SaveStatus;
  ...
  /**
   * How many transient failures have followed each other without a save in between. The
   * notice a failure deserves depends on this number and on nothing else.
   */
  failures: number;
  /** Save now rather than waiting out the backoff — the retry a failure offers. */
  retry: () => void;
  /** Write on top of the version that won the race, keeping what is on screen. */
  overwrite: () => void;
 ...
 */
export function sameDocument(a: SentenceSchemaDocument, b: SentenceSchemaDocument): boolean {
  if (a === b) return true;
  const keys = new Set([...Object.keys(a), ...Object.keys(b)]) as Set<keyof SentenceSchemaDocument>;
 ...
 *
 * A copy of `use-short-answer-autosave.ts` in everything that matters, because what
 * matters is what happens when a save does *not* work, and that is a property of the
 * platform rather than of the template. A failed save keeps the optimistic state and
 * retries with backoff; a conflict stops retrying and hands the decision to the teacher,
 * because the one thing that must never happen is their work disappearing into a write
 * that lost a race.
 *
 ...
 */
export function useSentenceSchemaAutosave({
  exerciseId,
  containerId,
  exercise,
  onSaved,
}: Options): SentenceSchemaAutosave {
  ...
  const flush = useCallback(
    async (force?: { expectedUpdatedAt: string }) => {
      ...
      const result = await saveSentenceSchemaAction(exerciseId, containerId, {
        ...
        // author for the same sentence twice would be a field with no question.
      ...
      if (!result.ok) {
        if (PERMANENT.has(result.error.code)) {
          // Nothing is scheduled. The edit stays on screen — that part never changes — but
          // the author is told this document was refused rather than watching a retry loop
          // describe it as a hiccup.
          attempt.current = 0;
      ...
      const outcome: SaveSentenceSchemaOutcome = result.value;
  ...
  // Kept current after every render, and declared before the effect that schedules a save
  // so that a flush always reads the newest document rather than the one from the render
  // that queued it.
  useEffect(() => {
