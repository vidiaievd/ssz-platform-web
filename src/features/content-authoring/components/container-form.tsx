...
export function ContainerForm(props: ContainerFormProps) {
  ...
  const {
    ...
    defaultValues: container
      ? {
          ...
          targetLanguage: container.targetLanguage as ContainerFormValues['targetLanguage'],
  ...
  const languageCtrl = useController({ name: 'targetLanguage', control });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="max-w-2xl space-y-5">
      ...
      >
        <Select
          value={languageCtrl.field.value ?? ''}
          onValueChange={languageCtrl.field.onChange}
          disabled={isPending}
        >
